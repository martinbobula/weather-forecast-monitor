DROP TABLE IF EXISTS alert_wind_next6h_decision;

CREATE TABLE alert_wind_next6h_decision AS
WITH run_ctx AS (
  SELECT
    now() AS run_time_utc,
...
wind_max AS (
  SELECT
    ...
    MAX(h.wind_kmh) AS max_wind_next6h
...
wind_pivot AS (
  SELECT
    ...
    MAX(CASE WHEN rn = 1 THEN max_wind_next6h END) AS current_max_wind_next6h,
    MAX(CASE WHEN rn = 2 THEN max_wind_next6h END) AS prev_max_wind_next6h
  FROM wind_max
)
SELECT
  run_time_utc,
  ...
  prev_snapshot_utc,
  current_max_wind_next6h,
  prev_max_wind_next6h,
  CASE
    WHEN prev_max_wind_next6h IS NOT NULL
     AND current_max_wind_next6h >= 30
     AND (current_max_wind_next6h - prev_max_wind_next6h) >= 15
    THEN TRUE
    ELSE FALSE
  END AS should_alert_wind_next6h
FROM wind_pivot;