DROP TABLE IF EXISTS alert_wind_next6h_gated;

CREATE TABLE alert_wind_next6h_gated AS
WITH decision AS (
  SELECT *
  FROM alert_wind_next6h_decision
  LIMIT 1
),
last_sent AS (
  SELECT
...
SELECT
  ...
  c.in_cooldown,
  CASE
    WHEN d.should_alert_wind_next6h = TRUE AND c.in_cooldown = FALSE THEN TRUE
    ELSE FALSE
